...
Status UnpackData(const uint8_t* buf, Frame* f) {
  ...
  PX_RETURN_IF_ERROR(ProcessPadding(&p));
...
Status UnpackHeaders(const uint8_t* buf, Frame* f) {
  ...
  PX_RETURN_IF_ERROR(ProcessPrioritySetting(&frame, &p));
...
RecordsWithErrorCount<Record> ProcessFrames(std::deque<Frame>* req_frames,
  ...
  std::map<uint32_t, http2k::HTTP2Message> reqs;
  std::map<uint32_t, http2k::HTTP2Message> resps;
  ...
  std::vector<http2k::Record> records = MatchGRPCReqResp(std::move(reqs), std::move(resps));
  ...
  http2k::EraseConsumedFrames(req_frames);
  http2k::EraseConsumedFrames(resp_frames);
