#include "src/stirling/source_connectors/socket_tracer/protocols/http2k/parse.h"
...
size_t FindFrameBoundaryForGRPCReq(std::string_view buf) {
  if (buf.size() < http2k::kFrameHeaderSizeInBytes + kGRPCReqMinHeaderBlockSize) {
    return std::string_view::npos;
  }
  for (size_t i = 0; i <= buf.size() - http2k::kFrameHeaderSizeInBytes - kGRPCReqMinHeaderBlockSize;
       ++i) {
...
constexpr size_t kRespHeadersFrameMinSize = http2k::kFrameHeaderSizeInBytes + 1;
...
bool LooksLikeHeadersFrameForGRPCResp(std::string_view buf) {
  ...
  if (buf[http2k::kFrameHeaderSizeInBytes + header_block_offset] != kStatus200) {
    return false;
  }
  return true;
}
...
size_t FindFrameBoundaryForGRPCResp(std::string_view buf) {
  ...
  for (size_t i = 0; i <= buf.size() - http2k::kFrameHeaderSizeInBytes - 1; ++i) {
