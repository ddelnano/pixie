...
DEFINE_int32(
    ...
    "If true, stirling will trace and process TLS protocol (not the TLS payload) messages.");
...
void SocketTraceConnector::AppendMessage(ConnectorContext* ctx, const ConnTracker& conn_tracker,
  ...
  r.Append<r.ColIndex("version")>(static_cast<uint64_t>(resp_message.legacy_version));
