...
#define VENDOR_SYMBOL(symbol) absl::StrCat(vendor_prefix, symbol)
  ...
  // Arguments of golang.org/x/net/http2/hpack.(*Encoder).WriteField.
  {
    ...
    auto write_field_e_loc = GetArgOffset(fn, "e", "0.37.0").ValueOr(invalid_loc);
    LOG_ASSIGN(symaddrs->WriteField_e_loc, write_field_e_loc);

    auto write_field_f_loc = GetArgOffset(fn, "f", "0.37.0").ValueOr(invalid_loc);
...
StatusOr<struct go_common_symaddrs_t> GoCommonSymAddrs(ElfReader* elf_reader,
  ...
  // TODO(ddelnano): This should be google.golang.org version
  LOG_ASSIGN_STATUSOR(symaddrs.syscallConn_conn_offset, GetStructOffset("google.golang.org/grpc/credentials/internal.syscallConn", "conn", go_version));
...
StatusOr<struct go_http2_symaddrs_t> GoHTTP2SymAddrs(ElfReader* elf_reader,
  ...
  PX_RETURN_IF_ERROR(PopulateHTTP2DebugSymbols(dwarf_reader, go_version, vendor_prefix, &symaddrs));
