...
#include "src/stirling/source_connectors/perf_profiler/symbolizers/go.h"
...
StatusOr<std::unique_ptr<Symbolizer>> GoSymbolizer::Create() {
  GoSymbolizer* elf_symbolizer = new GoSymbolizer();
  auto symbolizer = std::unique_ptr<Symbolizer>(elf_symbolizer);
  return symbolizer;
}

void GoSymbolizer::DeleteUPID(const struct upid_t& upid) { symbolizers_.erase(upid); }

namespace {

StatusOr<std::unique_ptr<GoSymbolizer::GoSymbolizerWithConverter>> CreateUPIDSymbolizer(
  ...
  PX_ASSIGN_OR_RETURN(auto symbolizer, GetGoSymbolizer(elf_reader));
  return std::make_unique<GoSymbolizer::GoSymbolizerWithConverter>(std::move(symbolizer));
}
...
profiler::SymbolizerFn GoSymbolizer::GetSymbolizerFn(const struct upid_t& upid) {
  ...
  std::unique_ptr<GoSymbolizerWithConverter>& symbolizer_with_converter = symbolizers_[upid];
  ...
  return absl::bind_front(&GoSymbolizer::GoSymbolizerWithConverter::Lookup,
                          symbolizer_with_converter.get());
}

std::string_view GoSymbolizer::GoSymbolizerWithConverter::Lookup(uint64_t virtual_addr) const {
  PX_UNUSED(virtual_addr);
