...
/**
 * @brief ExplodeNode splits a column containing delimited values into multiple rows.
 *
 * Similar to Apache Spark's explode function. Given an input row with a column
 * containing "x\ny\nz", this node produces three output rows with "x", "y", and "z"
 * respectively in that column position.
 */
class ExplodeNode : public ProcessingNode {
 public:
  ExplodeNode() = default;
  virtual ~ExplodeNode() = default;
 ...
 private:
  std::unique_ptr<plan::ExplodeOperator> plan_node_;
};
