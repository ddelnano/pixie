...
func (c *NatsErrorCounter) HandleNatsError(conn *nats.Conn, sub *nats.Subscription, err error) {
	...
	msgKind := extractMessageInfo(sub.Subject)
	switch err {
	case nats.ErrSlowConsumer:
		c.count.WithLabelValues(msgKind, "ErrSlowConsumer").Inc()
	default:
		c.count.WithLabelValues(msgKind, "ErrUnknown").Inc()
	}
}

func extractMessageInfo(subject string) string {
