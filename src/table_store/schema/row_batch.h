 ...
 */
template <typename T, typename = std::enable_if_t<std::is_same_v<T, arrow::Array> || std::is_same_v<T, arrow::ChunkedArray>>>
class RowBatch {
 public:
  /**
   * Creates a row batch.
   ...
   */
  RowBatch<T>(RowDescriptor desc, int64_t num_rows) : desc_(std::move(desc)), num_rows_(num_rows) {
    columns_.reserve(desc_.size());
  }

  Status ToProto(table_store::schemapb::RowBatchData* proto) const {
...
#undef TYPE_CASE
  ...
  static StatusOr<std::unique_ptr<RowBatch<T>>> FromProto(
...
#undef TYPE_CASE
    ...
    std::unique_ptr<RowBatch<T>> output_rb = std::make_unique<RowBatch<T>>(desc, proto.num_rows());
  ...
  static StatusOr<std::unique_ptr<RowBatch<arrow::Array>>> FromColumnBuilders(
   ...
   */
  static StatusOr<std::unique_ptr<RowBatch<arrow::Array>>> WithZeroRows(const RowDescriptor& desc, bool eow,
                                                          bool eos) {
   ...
   */
  StatusOr<std::unique_ptr<RowBatch<T>>> Slice(int64_t offset, int64_t length) const {
   ...
   */
  Status AddColumn(const std::shared_ptr<T>& col) {
   ...
   */
  std::shared_ptr<T> ColumnAt(int64_t i) const { return columns_[i]; }

  /**
   * @ param i the index of the column to check.
   * @ returns whether the rowbatch contains a column at the given index.
   */
  bool HasColumn(int64_t i) const { return columns_.size() > static_cast<size_t>(i); }
  ...
  std::vector<std::shared_ptr<T>> columns() const { return columns_; }
 ...
 private:
  ...
  bool eow_ = false;
  bool eos_ = false;
  std::vector<std::shared_ptr<T>> columns_;
};
